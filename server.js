...
app.post('/api/playlists', isAuthenticated, (req, res) => {
    ...
    const user = req.session.user;
    const ts = new Date().toISOString();
    db.run(`INSERT INTO playlists (user_id, name, created_at) VALUES (?, ?, ?)`, [user.id, name.trim(), ts], function(err) {
        if (err) return res.status(500).json({ error: 'Failed to create playlist' });
        res.status(201).json({ id: this.lastID, name: name.trim(), created_at: ts });
    });
});

app.get('/api/playlists', isAuthenticated, (req, res) => {
    const user = req.session.user;
    db.all(`SELECT p.*, (SELECT COUNT(*) FROM playlist_videos pv WHERE pv.playlist_id = p.id) as video_count FROM playlists p WHERE p.user_id = ? ORDER BY p.created_at DESC`, [user.id], (err, rows) => {
...
app.delete('/api/playlists/:id', isAuthenticated, (req, res) => {
    ...
    const user = req.session.user;
    db.get(`SELECT user_id FROM playlists WHERE id = ?`, [id], (err, row) => {
        ...
        db.run(`DELETE FROM playlists WHERE id = ?`, [id], (err) => {
            ...
            db.run(`DELETE FROM playlist_videos WHERE playlist_id = ?`, [id], (err) => {
                if (err) console.error('Failed to clean up playlist videos:', err);
            });
...
app.post('/api/playlists/:id/videos', isAuthenticated, (req, res) => {
    ...
    db.get(`SELECT user_id FROM playlists WHERE id = ?`, [playlistId], (err, row) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        if (!row) return res.status(404).json({ error: 'Playlist not found' });
        if (row.user_id !== user.id && user.role !== 'admin') return res.status(403).json({ error: 'Forbidden' });
        
        db.run(`INSERT OR IGNORE INTO playlist_videos (playlist_id, video_id, added_at) VALUES (?, ?, ?)`, [playlistId, video_id, ts], function(err) {
            if (err) return res.status(500).json({ error: 'Failed to add video to playlist' });
            res.status(201).json({ message: 'Video added to playlist' });
        });
    });
});

app.delete('/api/playlists/:id/videos/:videoId', isAuthenticated, (req, res) => {
    ...
    db.get(`SELECT user_id FROM playlists WHERE id = ?`, [id], (err, row) => {
        ...
        db.run(`DELETE FROM playlist_videos WHERE playlist_id = ? AND video_id = ?`, [id, videoId], function(err) {
            if (err) return res.status(500).json({ error: 'Failed to remove video from playlist' });
            res.json({ message: 'Video removed from playlist' });
        });
    });
});

app.get('/api/playlists/:id/videos', isAuthenticated, (req, res) => {
    const id = req.params.id;
    const user = req.session.user;
    ...
    db.get(`SELECT * FROM playlists WHERE id = ?`, [id], (err, row) => {
        if (err) return res.status(500).json({ error: 'Database error' });
        if (!row) return res.status(404).json({ error: 'Playlist not found' });
        // Optionally restrict viewing to owner/admin, or make public
        if (row.user_id !== user.id && user.role !== 'admin') return res.status(403).json({ error: 'Forbidden' });
        
        const sql = `SELECT v.* FROM videos v JOIN playlist_videos pv ON v.id = pv.video_id WHERE pv.playlist_id = ? ORDER BY pv.added_at DESC`;
        db.all(sql, [id], (err, rows) => {
            if (err) return res.status(500).json({ error: 'Failed to fetch playlist videos' });
            res.json(rows);
        });
    });
});
